...
(define (list-ref/protect l pos who)
  (with-handlers 
      ([exn:fail? 
        (lambda (x)
          (error 'list-ref/protect "~a out of bounds in list of len ~a, called by ~a. List: ~S"
                 pos (length l) who l))])
    (list-ref l pos)))
  ...
  (match top
    [(struct compilation-top (max-let-depth prefix form))
     (let-values ([(globs defns) (decompile-prefix prefix)])
       (unless (empty? defns)
         (error 'decompile "Can't decompile with toplevel defns"))
...
(define (decompile-form form globs stack closed)
  (match form
    ...
    [(struct def-syntaxes (ids rhs prefix max-let-depth))
     (error 'decompile-form "Syntax not supported")]
...
(define (extract-id expr)
  (match expr
    [(struct lam (name flags num-params arg-types rest? closure-map closure-types max-let-depth body)) ; Q: are closure types needed below?
     (extract-name name)]
...
(define (decompile-expr expr globs stack closed)
  (match expr
    ...
    [(struct apply-values (proc args-expr))
     `(call-with-values (lambda () ,(decompile-expr args-expr globs stack closed))
                        ,(decompile-expr proc globs stack closed))]
    [(struct seq (exprs))
    ...
    [(struct closure (lam gen-id))
     (if (hash-ref closed gen-id #f)
         ...
         (begin
           ...
           (decompile-expr lam globs stack closed)))]
    [(struct indirect (val))
     (if (closure? val)
         (decompile-expr val globs stack closed)
         '???)]
    [(struct req (reqs dummy))
     `(#%require ,@(map (lambda (r) `(quote ,r)) reqs))]
    ; XXX hack
    [(? void?)
     '(void)]
    [else `(quote ,expr)]))
...
(define (sanitize-name n)
  (match n
    [(? symbol?) n]
    [(vector name file a b c d e)
     (vector name (path->string file) a b c d e)]))
...
(define (decompile-lam expr globs stack closed)
  (match expr
    ...
    [(struct lam (name flags num-params arg-types rest? closure-map closure-types max-let-depth body)) ; Q: do we need closure-types below?
